...
public class TeaherAttendanceFXML implements Initializable {
    ...
    private void fillTable() {
        ...
        try {
            ...
            ResultSet set = db.getStatement().executeQuery("Select STUDENT_NAME, usn,currEng,totalEng,currMaths," +
                    "totalMaths,currSci,totalSci from STUDENT_INFO,attendance " +
                    "where attendance.link_id = STUDENT_INFO.link_id GROUP BY attendance.link_id;");
            while (set.next())
            ...
            if (count.next()) {
                ...
                if (val != 0) {
                    ResultSet set = db.getStatement().executeQuery("select CurrEng,CurrMaths,CurrSci from " +
                            "STUDENT_INFO,attendance where attendance.link_id = " + id);
                    if (set.next()) {
